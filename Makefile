...
CANDIDATE ?= "dev"
CONTAINER_BANKID ?= "igrant_bankid_dev"

CONTAINER_DEFAULT_RUN_FLAGS := \
	--rm $(TERM_FLAGS) \
	$(EXTRA_RUN_ARGS) \
	-v $(CURDIR)/resources/config/:/opt/igrant/bankid/config/:ro \
...
run: ## Run bankid locally for development purposes
	docker run \
		$(CONTAINER_DEFAULT_RUN_FLAGS) \
		--expose 80 \
		-e VIRTUAL_HOST=$(APP).$(PROJECT).dev \
		--name "${CONTAINER_BANKID}" \
		$(DOCKER_IMAGE):dev
...
deploy/%: $(DEPLOY_VERSION_FILE) ## Deploy to K8s cluster (e.g. make deploy/{preview,staging,production})
	kubectl set image deployment/igrant-bankid igrant-bankid=$(DEPLOY_VERSION)
